...
/*   Updated: 2017/03/17 06:09:18 by gbourgeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		do_this(char **reply, char *cmd, t_fd *cl)
{
	send(cl->fd, " ", 1, 0);
	send(cl->fd, reply[0], sv_size(reply[0]), 0);
	if (reply[1])
	{
		send(cl->fd, " ", 1, 0);
		send(cl->fd, reply[1], sv_size(reply[1]), 0);
	}
...
void			sv_err(char *err, char *cmd, char *cmd2, t_fd *cl)
{
	...
	if (pos > 0 && pos < 102)
	{
		...
		if (replies[pos][0])
			do_this(replies[pos], cmd, cl);
		else if (cmd)
			do_that(replies[pos], cmd, cmd2, cl);
		if (replies[pos][2])
		{
			send(cl->fd, " ", 1, 0);
