...
/*   Updated: 2018/05/14 11:47:10 by root             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Extra: Change the offset of sections higher than our code offset, for debug. */
	for (size_t i = 0; i < elf->header->e_shnum; i++) {
		if (elf->section[i].sh_offset >= elf->woody_program->p_offset + elf->woody_program->p_memsz) {
			elf->section[i].sh_offset += (woody32_size + e->woody_datalen);
		}
	}
...
/* 4. Change the elf header */
	...
	elf->header->e_shoff += (woody32_size + e->woody_datalen);

/* 5. Change the program header */
	if ((elf->woody_program->p_flags & PF_X) == 0)
		elf->woody_program->p_flags |= PF_X;
	elf->woody_program->p_memsz += (woody32_size + e->woody_datalen);
...
static void		write_new_file(t_env *e, t_elf32 *elf)
{
	...
	if (e->fd == -1)
		ft_fatal(NULL, e);
	ptr = (char *)e->file;
	off = elf->woody_program->p_offset + elf->woody_program->p_memsz - woody32_size - e->woody_datalen;
